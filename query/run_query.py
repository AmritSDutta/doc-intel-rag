...
def search_and_synthesize(query: str, n_results=3, cfg=None):
    ...
    contexts = [f"{m.get('source', 'unknown')}: {d}" for d, m in zip(docs, metas)]
    ...
    try:
        ...
        rel_res = rel_eval.evaluate(query=query, response=response_text, contexts=contexts)
    ...
    try:
        ...
        faith_res = faith_eval.evaluate(query=query, response=response_text, contexts=contexts)
